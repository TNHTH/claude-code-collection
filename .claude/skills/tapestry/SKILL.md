---
name: tapestry
description: 统一内容提取和行动规划。使用场景：用户说"tapestry <URL>"、"weave <URL>"、"help me plan <URL>"、"extract and plan <URL>"或类似短语。自动检测内容类型（YouTube视频、文章、PDF）并相应处理。将内容保存到 ~/Documents/claude/extracted-content/，计划保存到 ~/Documents/claude/plans/。
...
```
Content files → ~/Documents/claude/extracted-content\
Plan files    → ~/Documents/claude/plans\
