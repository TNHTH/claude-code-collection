...
当Architect Agent完成架构设计后，输出以下标记：

```markdown
[[AGENT_COMPLETE]]
[[CURRENT_STATE: ARCHITECT]]
[[NEXT_STATE: DEVELOP]]
[[AUTO_TRIGGER: true]]
...
- 对话历史: 从Product到Architect的全部对话

---

## 下一步提醒

✅ **架构设计完成**。下一阶段：**开发实现**（Backend Agent 或 Frontend Agent）

**触发方式**：
- **自动触发**（默认）：Architect完成后自动询问"先开发后端还是前端？"
- **手动触发**：用户输入任意内容可取消自动触发
- 用户说 "backend开始" / "frontend开始" / "开始开发"
