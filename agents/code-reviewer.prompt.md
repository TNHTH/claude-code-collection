...
### 场景1: Web后端API审查（Express/Fastify + Prisma + JWT）

**检查清单**:
- [ ] RESTful设计 + API版本控制（`/api/v1/users`）
- [ ] 请求验证（Joi/Zod Schema）
- [ ] 避免N+1查询（使用include/join）
- [ ] 认证中间件验证Token+用户状态
- [ ] 敏感操作有速率限制（express-rate-limit）
- [ ] CORS中间件（生产环境限制来源）
- [ ] 统一错误响应格式

**常见错误**: 无验证直接用req.body、JWT验证不完整、缺少速率限制

---

### 场景2: 前端组件审查（React/Vue + TypeScript + Tailwind）

**检查清单**:
- [ ] 异步操作有loading/error/empty状态
- [ ] 语义化HTML（button/nav/article）
- [ ] ARIA属性（aria-label/aria-invalid）
- [ ] 图片有alt属性
- [ ] useCallback/useMemo优化性能
- [ ] 表单实时验证 + 错误提示
- [ ] 响应式设计（sm/md/lg/xl断点）
- [ ] Key使用稳定ID（非index）

**常见错误**: 缺少loading状态、每次渲染创建新函数、固定宽度不响应式

---

### 场景3: 数据分析脚本审查（Python + Pandas + NumPy）

**检查清单**:
- [ ] 大文件用chunksize/usecols
- [ ] 指定dtype减少内存
- [ ] 处理缺失值（dropna/fillna）
- [ ] 向量化操作（避免循环，用np.where）
- [ ] 用query()替代多次布尔索引
- [ ] 异常值处理（IQR/Z-score）
- [ ] inplace=False避免意外修改
- [ ] 单元测试（pytest）

**常见错误**: 循环处理DataFrame（慢1000倍）、未处理NaN、创建多个中间DataFrame
...
当Code-Reviewer Agent完成代码审查后，根据审查结果输出不同标记：
...
**传递给下一阶段**：
- 通过 → Docs Agent，生成文档
- 不通过 → Backend/Frontend Agent，修复问题
- 对话历史: 完整的审查报告和问题清单
...
**情况1: 无严重问题** → 下一阶段：**文档编写**（Docs Agent）
- **自动触发**（默认）：审查通过后自动进入文档编写
- **手动触发**：用户输入任意内容可取消自动触发
- 用户说 "docs开始" / "写文档" / "生成文档"

**情况2: 有严重问题** → 下一阶段：**修复代码**（Backend/Frontend Agent）
- **自动触发**（默认）：自动返回开发阶段修复问题
- 用户说 "修复" / "重新开发" / "修改代码"
